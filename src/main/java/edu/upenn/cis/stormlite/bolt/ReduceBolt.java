...
import edu.upenn.cis455.mapreduce.BDBClass;
...
public class ReduceBolt implements IRichBolt {
    ...
    public void prepare(Map<String,String> stormConf, 
        ...
        dbPath = WorkerServer.getStoredDir() + "/" + getExecutorId();
        storageDB = new BDBClass(dbPath);
        ...
        neededVotesToComplete = mapExecutors + mapExecutors * (numOfWorkers -1);
    ...
    public synchronized void execute(Tuple input) {
    	if (sentEof) {
			...
			WorkerServer.setWaitRecord();
 	        WorkerServer.setReduceRecord();
 		    for(String key : storageDB.getWords()){
 				//System.out.println("Database retrieve: " + key + " ");
 				reduceJob.reduce(key, storageDB.getCount(key).iterator(), collector);
 				WorkerServer.addResults(key + "-" + collector.getPairValue(key));
 				WorkerServer.addKeysWritten();
    	} 
 		collector.emitEndOfStream();
		WorkerServer.setEndRecord();
 		sentEof = true;
 		storageDB.close();
        clearDB(new File(dbPath)); 
		}
 		    else {
	        ...
	        storageDB.addWord(key, value);
	        //System.out.println("[ReduceBolt] Processing key: " + key + " and value " + value + "         " + getExecutorId());
